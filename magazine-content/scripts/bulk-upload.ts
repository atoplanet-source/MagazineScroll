/**
 * Bulk Upload Script - Uploads all stories to Supabase
 *
 * Usage:
 *   cd magazine-content
 *   deno run --allow-read --allow-net --allow-env scripts/bulk-upload.ts
 */
...
async function uploadStories() {
  console.log(`Uploading ${stories.length} stories to Supabase...\n`);

  for (const story of stories) {
    console.log(`Uploading: ${story.title}...`);
    ...
    if (error) {
      ...
      // Also upsert content block
      await supabase.from("content_blocks").delete().eq("story_id", data.id);
      ...
      const { error: blockError } = await supabase
        ...
        .insert({
          ...
          position: 0,
          block_type: "text",
          text_content: story.body_text,
          text_color: "#FFFFFF",
          background_color: "#000000",
        });

      if (blockError) {
        console.error(`  Content block error: ${blockError.message}`);
      }
    }
  }
